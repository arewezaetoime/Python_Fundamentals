def input_validation(type_val, name_val, dmg_val, health_val, armor_val):
    if type_val[0].isupper() and name_val[0].isupper():
        if dmg_val in range(0, 100_001) and health_val in range(0, 100_001) and\
                armor_val in range(0, 100_001):
            return True
    return False
    ...
    for _ in range(n):
        ...
        if len(dragon_input) == 5:
            if 'null' not in dragon_input:
                ...
                if input_validation(str(type_dragon), str(name_dragon), int(damage_dragon), int(health_dragon),
                                    int(armor_dragon)):
                ...
                # Validating the data and then passing it to ADD function
                if input_validation(str(type_dragon), str(name_dragon), int(damage_dragon), int(health_dragon), int(armor_dragon)):
                    add_dragons(str(type_dragon), str(name_dragon), int(damage_dragon), int(health_dragon), int(armor_dragon))
